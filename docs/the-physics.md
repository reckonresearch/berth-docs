...
prompt length on two different cards under vLLM, and about 1.01 on an L40S under
SGLang. Two independent schedulers, both essentially serial, and to within a few
percent the same constant. The term is a property of batched inference rather
than of one scheduler.

One qualification, measured rather than assumed. Fitting `c_eff` separately at
each batch level gives roughly 1.0 at batch 4 through 16 and falls to 0.72 to
0.78 at batch 32 on both stacks. Prefill is serial at moderate concurrency and
partially overlapped at high concurrency. Modelling it as one constant is why
batch-32 residuals carry structure, which is disclosed on the validation page
and open.

Fit `c_eff` from your own traces rather than carrying this number. Subtract the
fixed floor first: a ratio taken from raw first-token latency inherits a
constant that does not scale, and doing so has produced a spurious 1.45 for this
very quantity. This is deterministic
